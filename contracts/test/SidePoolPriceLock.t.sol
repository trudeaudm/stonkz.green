...
abstract contract SidePoolPriceLockBase is V4DualBackend, LadderVectorLoader {
    ...
    function _listEth(bytes32 userSalt) internal returns (StonkzDirectListing l) {
        StonkzExpressFactory f =
            new StonkzExpressFactory(pm, locker, hook, acc, gov, ETH, address(sideTok));
        l = f.list{value: 1 ether}(_listingParams(), userSalt);
    }

    function _listUsdg(bytes32 userSalt) internal returns (StonkzDirectListing l) {
        StonkzExpressFactory f =
        ...
        l = f.list{value: 0}(_listingParams(), userSalt);
    }
    ...
    function test_lock_express_eth_slot0_matches_refprice() public {
        StonkzDirectListing l = _listEth(bytes32(uint256(1)));
    ...
    function test_lock_express_usdg_slot0_matches_refprice() public {
        StonkzDirectListing lEth = _listEth(bytes32(uint256(1)));
        StonkzDirectListing lUsdg = _listUsdg(bytes32(uint256(2)));
        ...
        emit log_named_string("tokIs0 flipped vs ETH salt1", ethTokIs0 != usdgTokIs0 ? "yes" : "no");
    ...
    function test_lock_vacuity_wrongPriceFailsMatch() public {
        StonkzDirectListing l = _listEth(bytes32(uint256(7)));
