...
///         at the correct auction blocks (spec §3). Demand basis stays distinct (§4).
/// @dev Scenario twin of reference engine (sizeBonus 17.85%, multi-bid A + competitor C):
///        A: $30 @ ∞  +  $200 @ 5.5  ;  C: $500 @ ∞
///      After block 1 price ≈ 5.5125 → block 2 price-out of $200 BEFORE fills → basis 30.
///      $30 goes all-in during a later clear → basis 0 from the following block.
contract Regression005 is Test {
    ...
    function test_weightBasis_allInThenPriceOut_timing() public {
        ...
        // Block 1 clear → price steps above 5.5
        _step();
        ...
        // Block 2 clear: price-out $200 BEFORE fills → basis → 30 for remainder / next
        _step();
        ...
        // Advance until $30 position is all-in (OutBudget) — basis must hit 0 the block AFTER
        bool sawAllIn;
