...
/// @title DeployExpressV4 — three-contract generation: adapter → FeeLocker → factory
/// @notice Reads singleton ctor args from live Express V3 except adapter+feeLocker (new).
///
/// ENV:
...
///   LADDER_SETTLEMENT - optional live settlement to authorize; if unset, skip settlement auth
///
/// Dry-run:
...
contract DeployExpressV4 is Script {
    ...
    function run() external {
        ...
        address ladderSettlement = vm.envOr("LADDER_SETTLEMENT", address(0));
        ...
        console2.log("=== EXPRESS V4 GEN - three-contract (adapter, FeeLocker, factory) ===");
        console2.log("SOURCE (V3) factory", sourceExpress);
        console2.log("canonical PM", RH_POOL_MANAGER);
        console2.log("hook (reuse)", address(hook));
        console2.log("accumulator (reuse)", address(accumulator));
        console2.log("ctoGovernor (reuse)", address(ctoGovernor));
        ...
        console2.log("ladderSettlement (authorize if set)", ladderSettlement);

        console2.log("=== SAFE PAYLOAD HINTS (post-broadcast; do NOT execute in dry-run) ===");
        console2.log("1) adapter.transferOwnership(Safe)");
        console2.log("2) factory.transferOwnership(Safe) if not DO_TRANSFER");
        console2.log("3) setRefPools: copy identical args from V3");
        ...
        console2.log("4) adapter.setAuthorized(factory, true) - done in this script as deployer");
        console2.log("5) adapter.setAuthorized(feeLocker, true) - done in this script");
        console2.log("6) adapter.setAuthorized(settlement, true) if LADDER_SETTLEMENT set");
        console2.log("7) accumulator.setPoolManager(newAdapter) if accumulator still points at old");
        ...
        address predictedAdapter = vm.computeCreateAddress(deployer, nonce);
        address predictedFeeLocker = vm.computeCreateAddress(deployer, nonce + 1);
        ...
        console2.log("predicted adapter", predictedAdapter);
        console2.log("predicted feeLocker", predictedFeeLocker);
        ...
        FeeLockerV2 feeLocker = new FeeLockerV2(IPoolManager(address(adapter)), hook);
        StonkzExpressFactory neu = new StonkzExpressFactory(
            IPoolManager(address(adapter)), feeLocker, hook, accumulator, ctoGovernor, pairToken, sideTokenRef
        );

        adapter.setAuthorized(address(neu), true);
        adapter.setAuthorized(address(feeLocker), true);
        if (ladderSettlement != address(0)) {
            adapter.setAuthorized(ladderSettlement, true);
        }

        if (doTransfer && broadcasting) {
            adapter.transferOwnership(custody);
        ...
        if (address(adapter) != predictedAdapter) {
            revert PredictMismatch("adapter", address(adapter), predictedAdapter);
        }
        if (address(feeLocker) != predictedFeeLocker) {
            revert PredictMismatch("feeLocker", address(feeLocker), predictedFeeLocker);
        }
        if (address(neu) != predictedFactory) {
            revert PredictMismatch("factory", address(neu), predictedFactory);
        }
        ...
        console2.log("adapter.manager", address(adapter.manager()));
        console2.log("feeLocker.poolManager", address(feeLocker.poolManager()));
        console2.log("factory.poolManager", address(neu.poolManager()));
        console2.log("factory.feeLocker", address(neu.feeLocker()));

        uint256 adapterSize = address(adapter).code.length;
        uint256 lockerSize = address(feeLocker).code.length;
        ...
        console2.log("adapter runtime size", adapterSize);
        console2.log("feeLocker runtime size", lockerSize);
        ...
        if (adapterSize > EIP170_MAX) revert SizeOverEip170("adapter", adapterSize);
        if (lockerSize > EIP170_MAX) revert SizeOverEip170("feeLocker", lockerSize);
        ...
        console2.log("factory owner", ownerNow);
        ...
        console2.log("authorized(factory)", adapter.authorized(address(neu)));
        console2.log("authorized(feeLocker)", adapter.authorized(address(feeLocker)));
        ...
        require(adapter.authorized(address(neu)), "gate: factory authorized");
        require(adapter.authorized(address(feeLocker)), "gate: feeLocker authorized");
        require(
            ownerNow == deployer || (doTransfer && broadcasting && ownerNow == custody), "gate: factory owner"
        );
        console2.log("soft-launch posture: CLOSED (deployer-only) - OK");
        console2.log("fresh-gate: new adapter+locker+factory; V3 untouched");

        if (doTransfer && broadcasting) {
            if (neu.owner() != custody) revert OwnerMismatch(neu.owner(), custody);
            if (adapter.owner() != custody) revert OwnerMismatch(adapter.owner(), custody);
        }
